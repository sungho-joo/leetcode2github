...
def init_variables_1722():
    from src.leetcode_1722_minimize_hamming_distance_after_swap_operations import (
        Solution,
    )
...
class TestClass1722:
    def test_solution_0(self, init_variables_1722):
        assert (
            init_variables_1722().minimumHammingDistance([1, 2, 3, 4], [2, 1, 4, 5], [[0, 1], [2, 3]])
            == 1
        )

    def test_solution_1(self, init_variables_1722):
        assert init_variables_1722().minimumHammingDistance([1, 2, 3, 4], [1, 3, 2, 4], []) == 2
