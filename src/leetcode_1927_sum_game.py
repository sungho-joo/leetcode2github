...
# For Bob to win,
# the sum of the digits in the first half of num must be equal to the sum of the digits in the second half.
...
class Solution:
    def sumGame(self, num: str) -> bool:
        ...
        smaller_qms = min(qms)
        qms = [q - smaller_qms for q in qms]
        ...
        for j in range(2):
            if sums[0] < sums[1]:
                ...
                num_decre, remainder = divmod(qms[0] - qms[1], 2)
                if remainder != 0:
                    return True

                # print(j, sums, qms, num_decre)
                if sums[1] - sums[0] == num_decre * 9:
                    return False
                else:
                    return True
            sums.reverse()
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1927.py")])
