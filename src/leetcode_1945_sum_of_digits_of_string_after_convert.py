...
# replace 'a' with 1,'b' with 2,...,'z' with 26).Then,
...
class Solution:
    def getLucky(self, s: str, k: int) -> int:
        string_nums = [str(ord(alp) - ord("a") + 1) for alp in s]
        next_num = int("".join(string_nums))
        for _ in range(k):
            print(next_num)
            string_nums = [int(num) for num in str(next_num)]
            next_num = sum(string_nums)
        return next_num
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1945.py")])
