...
class Solution:
    def longestObstacleCourseAtEachPosition(self, obstacles: List[int]) -> List[int]:
        long_list = [obstacles[0]]
        ans = [1]
        for i in range(1, len(obstacles)):
            if long_list[-1] <= obstacles[i]:
                long_list.append(obstacles[i])
                ans.append(len(long_list))
            else:
                ind = bisect.bisect(long_list, obstacles[i])
                long_list[ind] = obstacles[i]
                ans.append(ind + 1)
        return ans
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1964.py")])
