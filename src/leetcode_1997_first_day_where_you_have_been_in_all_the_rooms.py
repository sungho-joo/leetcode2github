...
# on the next day you will visit the room specified by nextVisit[i] where 0 <= nextVisit[i] <= i;
...
class Solution:
    def firstDayBeenInAllRooms(self, nextVisit: List[int]) -> int:
        ...
        for i in range(n - 1):
            dp[i + 1] = (2 * dp[i] - dp[nextVisit[i]] + 2) % (10 ** 9 + 7)
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1997.py")])
