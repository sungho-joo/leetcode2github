...
class Solution:
    def canMerge(self, trees: List[TreeNode]) -> TreeNode:
        ...
        for tree in trees:
            nodes[tree.val] = tree
            if tree.val not in indeg:
                indeg[tree.val] = 0
            if tree.left:
                if tree.left.val not in nodes:
                    nodes[tree.left.val] = tree.left
                indeg[tree.left.val] += 1

            if tree.right:
                if tree.right.val not in nodes:
                    nodes[tree.right.val] = tree.right
                indeg[tree.right.val] += 1

        # find source(no indegree)
        source = [key for key, val in indeg.items() if val == 0]

        # Check multiple source node
        if len(source) != 1:
            return None
        ...
        def transverse(val):
            ...
            seen.add(node)
            if node.left:
                node.left = transverse(node.left.val)
            ...
            self.cur = val
            if node.right:
                node.right = transverse(node.right.val)
            return node

        root = transverse(source[0])
        if len(seen) != len(nodes) or self.is_invalid is True:
            return
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1932.py")])
