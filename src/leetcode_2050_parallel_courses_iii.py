...
# Furthermore,
# you are given a 0-indexed integer array time where time[i] denotes how many months it takes to complete the (i+1)th course.
...
# Course 5 can be taken only after courses 1,2,3,and 4 have been completed,i.e.,after max(1,2,3,
# 7) = 7 months.
...
class Solution:
    def minimumTime(self, n: int, relations: List[List[int]], time: List[int]) -> int:
        indegree, node_map = defaultdict(int), defaultdict(list)
        ...
        while q:
            for _ in range(len(q)):
                ...
                cur_max = max(cur_max, cur_time + time[node - 1])
                for next_node in node_map[node]:
                    indegree[next_node] -= 1
                    helper[next_node].append(cur_time + time[node - 1])
                    if indegree[next_node] == 0:
                        q.append([next_node, max(helper[next_node])])
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_2050.py")])
