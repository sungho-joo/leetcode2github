...
# - Choose the first and last triplets [[2,5,3],[1,8,4],[1,7,5]].Update the last triplet to be [max(2,
# 1),max(5,7),max(3,5)] = [2,7,5].triplets = [[2,5,3],[1,8,4],[2,7,5]]
...
# Update the third triplet to be [max(2,1),max(5,2),max(3,5)] = [2,5,5].triplets = [[2,5,3],[2,3,4],
# [2,5,5],[5,2,3]].
# - Choose the third and fourth triplets [[2,5,3],[2,3,4],[2,5,5],[5,2,3]].
# Update the fourth triplet to be [max(2,5),max(5,2),max(5,3)] = [5,5,5].triplets = [[2,5,3],[2,3,4],
# [2,5,5],[5,5,5]].
...
# Output: false
# Explanation: It is impossible to have [3,2,
# 5] as an element because there is no 2 in any of the triplets.
#
#
# Constraints:
#
...
class Solution:
    def mergeTriplets(self, triplets: List[List[int]], target: List[int]) -> bool:
        get_max = lambda a, b: [max(a[i], b[i]) for i in range(3)]
        is_small = lambda a, b: a[0] <= b[0] and a[1] <= b[1] and a[2] <= b[2]
        ans = None
        for triplet in triplets:
            if triplet == target:
                return True
            if is_small(triplet, target):
                if ans:
                    ans = get_max(ans, triplet)
                    if ans == target:
                        return True
                else:
                    ans = triplet
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1899.py")])
