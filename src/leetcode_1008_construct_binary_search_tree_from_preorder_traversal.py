...
class Solution:
    def bstFromPreorder(self, preorder: List[int]) -> Optional[TreeNode]:
        def traverse(left, right):
            ...
            mid = left
            while mid < len(preorder) and preorder[mid] <= preorder[left]:
                mid += 1

            root.left = traverse(left + 1, mid - 1)
            root.right = traverse(mid, right)
            return root
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1008.py")])
