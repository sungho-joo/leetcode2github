...
class Solution:
    def maxCompatibilitySum(self, students: List[List[int]], mentors: List[List[int]]) -> int:
        # Naive get all combinations (max - 8!)
        get_score = lambda arr1, arr2: sum([int(n1 == n2) for n1, n2 in zip(arr1, arr2)])
        m, n = len(students), len(students[0])
        max_score = 0
        for perm in permutations(range(m)):
            score = 0
            for i in range(m):
                score += get_score(students[i], mentors[perm[i]])
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_1947.py")])
