...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        cooldown, hold, sell = 0, -float("inf"), 0
        for price in prices:
            cooldown, hold, sell = max(cooldown, sell), max(cooldown - price, hold), hold + price
        return max(cooldown, hold, sell)
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_309.py")])
