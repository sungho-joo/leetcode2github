...
# you need to check if all the numbers in s are strictly increasing from left to right (i.e.,
# other than the last number,each number is strictly smaller than the number on its right in s).
...
class Solution:
    def areNumbersAscending(self, s: str) -> bool:
        s = s.split()
        s = [int(alp) for alp in s if alp.isdecimal()]
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_2042.py")])
