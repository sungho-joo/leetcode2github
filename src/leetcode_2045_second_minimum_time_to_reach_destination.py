...
# The edges in the graph are represented as a 2D integer array edges,where each edges[i] = [ui,
...
class Solution:
    def secondMinimum(self, n: int, edges: List[List[int]], time: int, change: int) -> int:
        ...
        # Last visit time : 0 -> not visited, t -> visited once, -1 -> visited twice(t+1) and not allowed any more
        last_visit = [0] * (n + 1)
            ...
            for _ in range(len(q)):
                ...
                for next_node in node_map[node]:
                    ...
                    if rg_zone % 2 == 0:
                        ...
                        next_start = change * (rg_zone + 1)
                        arrival = next_start + time

                    if last_visit[next_node] == 0:
                        last_visit[next_node] = arrival
                        q.append([next_node, arrival])
                    elif last_visit[next_node] < arrival:
                        last_visit[next_node] = -1
                        q.append([next_node, arrival])
            # if self.min_time < float('inf'): break
...
#         cur_time = self.min_time
#         for _ in range(2):
#             rg_zone = cur_time // change
#             if rg_zone % 2 == 0:
#                 cur_time += time
#             else:
#                 next_start = change * (rg_zone+1)
#                 cur_time = next_start + time
#         return cur_time
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_2045.py")])
