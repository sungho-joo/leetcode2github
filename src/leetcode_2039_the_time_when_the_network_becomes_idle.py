...
# All servers are connected,i.e.,
# a message can be passed from one server to any other server(s) directly or indirectly through the message channels.
...
# Messages move between servers optimally,
# so every message takes the least amount of time to arrive at the master server.
...
# The data server i will resend the message every patience[i] second(s),i.e.,
# the data server i will resend the message if patience[i] second(s) have elapsed since the last time the message was sent from this server.
...
# - Server 1 has not received any reply.
# 1 second (1 < patience[1] = 2) elapsed since this server has sent the message,
# therefore it does not resend the message.
# - Server 2 has not received any reply.
# 1 second (1 == patience[2] = 1) elapsed since this server has sent the message,
...
class Solution:
    def networkBecomesIdle(self, edges: List[List[int]], patience: List[int]) -> int:
        def get_last_start(dist, ind):
            interval = patience[ind]
            a, b = divmod(dist, interval)
            if a == 0:
                return 0
            start_ind = a if b != 0 else a - 1
            return start_ind * interval
        ...
        for i in range(1, len(patience)):
            cur_dist = dist_map[i] * 2
            ans = max(ans, get_last_start(cur_dist, i) + cur_dist)
        return ans + 1
...
if __name__ == "__main__":
    ...
    pytest.main([os.path.join("tests", "test_2039.py")])
